...
class ScannetDataset(BaseDataset):
    def __init__(self, root_dir, split='train', downsample=1.0, only_pose=False, **kwargs):
        ...
        # root_dir needs to be [prefix]/iphone
        self.scannetpp_downsample_factor = 0.5
        pose_intrinsic_imu_path = os.path.join(self.root_dir, 'pose_intrinsic_imu.json')
        with open(pose_intrinsic_imu_path, 'r') as f_caminfo:
            self.pose_intrinsic_imu_info = json.load(f_caminfo)

        caminfo_path = os.path.join(self.root_dir, 'colmap', 'cameras.txt')
        with open(caminfo_path, 'r') as f_caminfo:
            f_caminfo.readline()
            f_caminfo.readline()
            f_caminfo.readline()
            line = f_caminfo.readline().strip(" \r\n").split(" ")
            self.img_wh = (int(line[2]), int(line[3]))
    ...
    def read_intrinsics(self, **kwargs):
        self.K = torch.FloatTensor(self.pose_intrinsic_imu_info["frame_000000"]["intrinsic"]).reshape(3, 3)
        w, h = self.img_wh
    ...
    def read_meta(self, split, **kwargs):
        ...
        self.poses, scale_factor, transform = auto_normalize_poses(self.poses,scale_factor=kwargs.get('scale_factor', 0.8))
        ...
        if split != 'test':
            if kwargs.get('normal_mono', False):
                ...
                for pi, path in enumerate(normal_paths):
                    ...
                    if self.scannetpp_downsample_factor < 1:
                        normal = normal.reshape(h, w, -1)
                        normal = cv2.resize(normal, (_w, _h))
